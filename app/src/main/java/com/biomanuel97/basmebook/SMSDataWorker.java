...
class SMSDataWorker {
    ...
    static void syncSMSTransactions(Context mContext, long mostRecent) {
        Uri inboxUri = Uri.parse("content://sms/inbox");
        ArrayList<String[]> transactionList = new ArrayList<>();
        ContentResolver contentResolver = mContext.getContentResolver();
        ...
        int numOfTrans = transactionList.size();
        for (int i = 1; i <= numOfTrans; i++){
            long time = Long.valueOf(transactionList.get(numOfTrans - i)[3]);
            if(time > mostRecent)
            UtilManager.getInstance().newTransaction(transactionList.get(numOfTrans - i)[0], transactionList.get(numOfTrans - i)[1],
                    transactionList.get(numOfTrans - i)[2], Long.valueOf(transactionList.get(numOfTrans - i)[3]), false);
        }
